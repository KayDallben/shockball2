...
class ContractForm extends React.Component {
    ...
    handleSubmit(event) {
      ...
      if (this.meetsMarketRules(data.get('purchasePrice'))) {
        this.saveContract(data.get('purchasePrice'))
      }

    }

    saveContract(purchasePrice) {
      const newContract = {
    ...
    render() {
      ...
      return (
        <form novalidate onSubmit={this.handleSubmit} id="contract" className={displayErrors ? 'displayErrors' : ''}>
          ...
          <input id="purchasePrice" name="purchasePrice" type="number" data-parse="purchasePrice" required />
